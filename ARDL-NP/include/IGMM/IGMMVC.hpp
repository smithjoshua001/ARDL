...
namespace ARDL {
    ...
    namespace NP {
        ...
        class IGMMVC {
            ...
            MatrixX<T> initCov, mt_m2;
            VectorX<T> outputAccumulator, mu, mt_diff;
            ...
            IGMMVC(const size_t totalDim, const size_t outputDim,
                ...
                lifeMin= 2 * totalDim;
                spMin= 1 + totalDim;
                initCov.resize(totalDim,totalDim);
                initCov.setZero();
                if(initialCov.cols() == 1) {
                    initCov.diagonal()= initialCov.array().inverse();
                } else {
                ...
                mu.resize(totalDim);
                mu.setZero();
                mt_diff.resize(totalDim);
                mt_m2.resize(totalDim,totalDim);
                mt_m2=initCov/2;
                count = 2;
            }

            IGMMVC(const IGMMVC<T>& copy):tmpComponent(MVN<T>(VectorX<T>::Zero(copy.dim),VectorX<T>::Zero(copy.dim)),T(0)) {
            ...
            void incrementalUpdate(const Eigen::MatrixBase<D> &x) {
                mt_diff = x - mu;
                mu += mt_diff/++count;//mt_diff*(x-mu).transpose()
                mt_m2-= (mt_m2*mt_diff*(x-mu).transpose()*mt_m2)/(1+(x-mu).transpose()*mt_m2*mt_diff);
                initCov.noalias() = mt_m2*count;
