...
template <typename T> class Chain {
  private:
    ...
    /**
             ...
             */
    void init(std::string root_name, std::string tip_name,
              urdf::ModelInterfaceSharedPtr model) {
        urdf::LinkConstSharedPtr m_rootLink = model->getLink(root_name);
        // Breadth first search for route between root and tip
        std::stack<urdf::LinkConstSharedPtr> t_urdfChain =
            Util::Search::searchBreadth(
                m_rootLink,
                /**
                     ...
                     * been expanded
                     */
                [](urdf::LinkConstSharedPtr currentLink,
                   std::queue<urdf::LinkConstSharedPtr> &searchQueue,
                   std::map<urdf::LinkConstSharedPtr, urdf::LinkConstSharedPtr>
                       &childParentMap) {
                    for (urdf::LinkConstSharedPtr childL :
                         currentLink->child_links) {
                        // if child is already found in map then it is expanded
                        if (childParentMap.find(childL) !=
                            childParentMap.end()) {
                            continue;
                        }
                        searchQueue.push(childL);
                        childParentMap.insert(
                            std::pair<urdf::LinkConstSharedPtr,
                                      urdf::LinkConstSharedPtr>(childL,
                                                                currentLink));
                    }
        ...
        if (t_urdfChain.size() == 0) {
            ...
            this->m_links_vsp.back().loadChildJoint(t_urdfChain.top()->name,
                                                    m_joints_all_vsp);
            ...
            while (!t_urdfChain.empty()) {
                ...
                this->m_links_vsp.emplace_back(
                    t_urdfChain.top(), &(this->m_links_vsp.back()),
                ...
                if (!t_urdfChain.empty()) {
                    ...
                    this->m_links_vsp.back().loadChildJoint(
                        t_urdfChain.top()->name, m_joints_all_vsp);
                } else {
                ...
                if (!ARDL_visit(this->m_links_vsp.back().getParentJoint(),
                                isFixed())) {
                    // if not fixed then add to moveable vector of pointers
                    m_jointNum++;
                    this->m_movable_links_vsp.push_back(
                        &(this->m_links_vsp.back()));
                    this->m_joints_vsp.push_back(
  ...
  public:
    ...
    Chain(const Chain<T> &copy) {
        ...
        for (size_t i = 1; i < copy.m_links_vsp.size(); i++) {
            std::ptrdiff_t parentLIndex =
                &(copy.m_links_vsp[i].getParentLink()) -
                copy.m_links_vsp.data();
            std::ptrdiff_t parentJIndex =
                &(copy.m_links_vsp[i].getParentJoint()) -
                copy.m_joints_all_vsp.data();
            ...
            ARDL_VISIT(
                ...
                [&](PrismaticJoint<T> &joint) {
                    m_joints_all_vsp.emplace_back(
                        PrismaticJoint<T>(joint, m_links_vsp[parentLIndex]));
                },
                [&](FixedJoint<T> &joint) {
                    m_joints_all_vsp.emplace_back(
...
#else
            ARDL_VISIT(
                overloaded{[&](const RevoluteJoint<T> &joint) {
                               m_joints_all_vsp.emplace_back(RevoluteJoint<T>(
                                   joint, m_links_vsp[parentLIndex]));
                           },
                           [&](const PrismaticJoint<T> &joint) {
                               m_joints_all_vsp.emplace_back(PrismaticJoint<T>(
                                   joint, m_links_vsp[parentLIndex]));
                           },
                           [&](const FixedJoint<T> &joint) {
                               m_joints_all_vsp.emplace_back(FixedJoint<T>(
                                   joint, m_links_vsp[parentLIndex]));
                           }},
                copy.m_joints_all_vsp[i - 1]);
#endif
            m_links_vsp.emplace_back(copy.m_links_vsp[i],
                                     &(m_joints_all_vsp[parentJIndex]),
            ...
            std::shared_ptr<RevoluteJoint<T>> joint_ptr =
                std::dynamic_pointer_cast<RevoluteJoint<T>>(
                    &(copy.m_joints_all_vsp[i - 1]));
            if (joint_ptr) {
                m_joints_all_vsp.push_back(RevoluteJoint<T>(
                    *(joint_ptr), &(m_links_vsp[parentLIndex])));
                ...
                std::shared_ptr<FixedJoint<T>> fixed_ptr =
                    std::dynamic_pointer_cast<FixedJoint<T>>(
                        &(copy.m_joints_all_vsp[i - 1]));
                if (fixed_ptr) {
                    m_joints_all_vsp.push_back(
                        FixedJoint<T>(*(joint_ptr), m_links_vsp[parentLIndex]));
                }
            }
            m_links_vsp.emplace_back(copy.m_links_vsp[0],
                                     &(m_joints_all_vsp.back()),
                                     &(m_links_vsp[parentLIndex]));
            if (joint_ptr) {
    ...
    /**
             ...
             */
    Chain(std::string root_name, std::string tip_name,
          urdf::ModelInterfaceSharedPtr model) {
    ...
    void randomPos() {
        ...
        for (size_t t_i = 0; t_i < m_jointNum; t_i++) {
            ...
            this->m_q.segment(currentIndex,
                              ARDL_visit(*(m_joints_vsp[t_i]), getDof())) =
    ...
    void updateChain(const Eigen::MatrixBase<qDerived> &q,
    ...
    void updateChainPos(const Eigen::MatrixBase<Derived> &q) {
        ...
        for (size_t t_i = 0; t_i < m_jointNum; t_i++) {
            ...
            ARDL_VISIT(
                *(m_joints_vsp[t_i]),
                [&t_i, &q](RevoluteJoint<T> &joint) {
                    joint.setQ(
                        q.template segment<RevoluteJoint<T>::getDof()>(t_i));
                },
                [&t_i, &q](PrismaticJoint<T> &joint) {
                    joint.setQ(
                        q.template segment<PrismaticJoint<T>::getDof()>(t_i));
                },
            ...
            m_joints_vsp[t_i]->setQ(
                q.segment(t_i, m_joints_vsp[t_i]->getDof()));
#endif
        }
        this->m_q = q;
    }
            ...
            ARDL_VISIT(
                *(m_joints_vsp[t_i]),
                [&t_i, &qd](RevoluteJoint<T> &joint) {
                    joint.setQd(
                        qd.template segment<RevoluteJoint<T>::getDof()>(t_i));
                },
                [&t_i, &qd](PrismaticJoint<T> &joint) {
                    joint.setQd(
                        qd.template segment<PrismaticJoint<T>::getDof()>(t_i));
                },
...
#endif
#else
            m_joints_vsp[t_i]->setQd(
                qd.segment(t_i, m_joints_vsp[t_i]->getDof()));
#endif
        }
    }
    /**
    ...
    void updateMatrices() {
        for (size_t t_i = 0; t_i < m_jointNum; t_i++) {
            ARDL_visit(*(m_joints_vsp[t_i]), update());
        }
    }
    /**
    ...
    void updateMatricesOptim() {
        for (size_t t_i = 0; t_i < m_jointNum; t_i++) {
            ARDL_visit(*(m_joints_vsp[t_i]), updateOptim());
        }
    ...
    void updateParams(const Eigen::MatrixBase<Derived> &paramDot) {
        for (size_t t_i = 0; t_i < m_jointNum; t_i++) {
            m_movable_links_vsp[t_i + 1]->updateParams(
    ...
    aligned_vector<Link<T> *> &getMoveableLinksRef() {
        return m_movable_links_vsp;
    }
