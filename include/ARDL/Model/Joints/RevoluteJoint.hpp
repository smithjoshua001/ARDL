...
ARDL_NTYPE_CLASS_INHERIT(T, RevoluteJoint, Joint) {
  public:
    RevoluteJoint() : ARDL_INHERIT_TYPE(T, RevoluteJoint, Joint)() {
        this->initDofSizes();
    }
    RevoluteJoint(urdf::JointSharedPtr input, Link<T> & parentLink)
        ...
        this->m_s.setZero();
        this->m_s.template segment<3>(3) << input->axis.x, input->axis.y,
            input->axis.z;
    ...
    void setQ(const Eigen::MatrixBase<qDerived> &q) {
        if (this->m_q[0] != q[0]) {
            ...
            jointTrans[1] =
                this->m_s(4) * this->m_s(3) * mcosq + this->m_s(5) * sinq;
            jointTrans[2] =
                this->m_s(5) * this->m_s(3) * mcosq - this->m_s(4) * sinq;
            jointTrans[3] =
                this->m_s(3) * this->m_s(4) * mcosq - this->m_s(5) * sinq;
            jointTrans[4] = cosq + this->m_s(4) * this->m_s(4) * mcosq;
            jointTrans[5] =
                this->m_s(5) * this->m_s(4) * mcosq + this->m_s(3) * sinq;
            jointTrans[6] =
                this->m_s(3) * this->m_s(5) * mcosq + this->m_s(4) * sinq;
            jointTrans[7] =
                this->m_s(4) * this->m_s(5) * mcosq - this->m_s(3) * sinq;
