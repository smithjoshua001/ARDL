...
namespace ARDL {
    ...
    template<typename T, Frame F= Frame::BODY>
    class ForwardKinematics {
        ...
        void getAdjoints(aligned_vector<AdjointSE3<T>> &ATs, bool update_collision= false) {
            ...
            for(Link<T> &link: m_chain->getLinksRef()) {
                ...
                if(update_collision) { link.updateCollision(last); }
            }
        }

        void getBodyAdjoints(aligned_vector<AdjointSE3<T>> &ATs, bool update_collision= false) {
            int t_j= ATs.size() - 1;
            AdjointSE3<T> &last= ATs.front();
            last.setIdentity();
            for(auto link= m_chain->getLinksRef().rbegin(); link != m_chain->getLinksRef().rend(); link++) {
                if(update_collision) { link->updateCollision(last); }
                if(!link->isRoot() && !ARDL_visit(link->getParentJoint(), isFixed())) {
                    size_t index= std::max(t_j, 0);
                    ATs[index]= last;
                    ARDL_visit(link->getParentJoint(), getAdjointLocal()).applyInverse(last, last);
                    t_j--;
                } else if(!link->isRoot()) {
                    ARDL_visit(link->getParentJoint(), getAdjointLocal()).applyInverse(last, last);
                }
        ...
        void getAdjointsOptim(aligned_vector<AdjointSE3<T>> &ATs, bool update_collision= false) {
            AdjointSE3<T> &last= ATs.back();
            last.setIdentity();
            ATs[0]= last;
            if(update_collision) { m_chain->getMoveableLinksRef()[0]->updateCollisionOptim(last); }
            for(size_t i= 1; i < m_chain->getMoveableLinksRef().size(); i++) {
                last.applyTo(ARDL_visit(m_chain->getMoveableLinksRef()[i]->getParentJoint(), getAdjointLocal()));
                ATs[i]= last;
                if(update_collision) { m_chain->getMoveableLinksRef()[i]->updateCollisionOptim(last); }
            }
        }

        void getEEAdjoint(aligned_vector<AdjointSE3<T>> &ATs, AdjointSE3<T> &ee) {
            if(ARDL_visit(m_chain->getEELinkRef().getParentJoint(), isFixed())) {
                ATs.back().applyTo(ARDL_visit(m_chain->getEELinkRef().getParentJoint(), getAdjointLocal()), ee);
            }
            else{
                ee = ATs.back();
            }
        ...
        void getBodyAdjointsOptim(aligned_vector<AdjointSE3<T>> &ATs, bool update_collision= false) {
            AdjointSE3<T> &last= ATs.front();
            last.setIdentity();
            for(size_t i= ATs.size() - 1; i > 0; i--) {
                ...
                if(update_collision) { m_chain->getMoveableLinksRef()[i]->updateCollisionOptim(last); }
                ARDL_visit(m_chain->getMoveableLinksRef()[i]->getParentJoint(), getAdjointLocal())
                    .applyInverse(last, last);
            }
            if(update_collision) { m_chain->getMoveableLinksRef()[0]->updateCollisionOptim(last); }
            // ARDL_visit(m_chain->getMoveableLinksRef()[0]->getParentJoint(), getAdjointLocal())
            //     .applyInverse(last, last);
            ATs[0]= last;
        }
        void getMixedAdjoints(aligned_vector<AdjointSE3<T>> &ATs, bool update_collision= false) {
            size_t t_j= 0;
            AdjointSE3<T> &last= ATs.back();
            last.setIdentity();
            ...
            for(size_t i= 0; i < ATs.size(); i++) { ATs[i].getPRef()-= ATs.back().getP(); }
        }

        template<Frame OF= F>
        void getJacobians(const aligned_vector<AdjointSE3<T>> &Ads, aligned_vector<Jacobian<T>> &jacs) {
            jacs.front().setZero();

            if constexpr(OF == Frame::BODY) {
                jacs[0].col(0).noalias()= ARDL_visit(m_chain->getMoveableLinksRef()[1]->getParentJoint(), getS());
                size_t dof= m_chain->getNumOfJoints();
                Jacobian<T> mt_jacobian(dof), mt_jac2(dof);
                mt_jacobian.col(0)= jacs[0].col(0);
                for(size_t i= 1; i < dof; i++) {
                    ARDL_visit(m_chain->getMoveableLinksRef()[i + 1]->getParentJoint(), getAdjointLocal())
                        .applyInverseMat(jacs[i - 1].template block<6, Eigen::Dynamic>(0, 0, 6, i),
                                         jacs[i].template block<6, Eigen::Dynamic>(0, 0, 6, i));
                    jacs[i].col(i).noalias()=
                        ARDL_visit(m_chain->getMoveableLinksRef()[i + 1]->getParentJoint(), getS());
                }

            } else if constexpr(OF == Frame::SPATIAL) {
                Jacobian<T> mt_jacobian(m_chain->getNumOfJoints());
                for(size_t i= 0; i < m_chain->getNumOfJoints(); i++) {
                    mt_jacobian.col(i)= ARDL_visit(m_chain->getMoveableLinksRef()[i + 1]->getParentJoint(), getS());
                }
                for(size_t i= 0; i < m_chain->getNumOfJoints(); i++) {
                    jacs[i].template block<6, -1>(0, 0, 6, i).noalias()= mt_jacobian.template block<6, -1>(0, 0, 6, i);
        ...
        template<Frame OF= F>
        void getJacobianDots(aligned_vector<AdjointSE3<T>> &Ads, aligned_vector<LieBracketSE3<T>> &adjs,
                             aligned_vector<Jacobian<T>> &jacs, aligned_vector<Jacobian<T>> &jacDots) {
            jacDots.front().setZero();
            if constexpr(OF == Frame::SPATIAL) {
                jacDots[0].template col(0)= jacs[0].template col(0);
                adjs[0].apply(jacDots[0].template col(0), jacDots[0].template col(0), mt_jacobianDot.template col(0));
                ...
                // AdjointSE3<T> mt_accumulatorLocal;
                // mt_accumulatorLocal.setIdentity();
                LieBracketSE3<T> mt_bodyVelocity;
                for(size_t i= 1; i < dof; i++) {
                    ARDL_visit(m_chain->getMoveableLinksRef()[i + 1]->getParentJoint(), getAdjointLocal())
                        .applyInverseMat(jacDots[i - 1].template block<6, Eigen::Dynamic>(0, 0, 6, i),
                                         jacDots[i].template block<6, Eigen::Dynamic>(0, 0, 6, i));

                    mt_bodyVelocity.getVelocity()=
                        ARDL_visit(m_chain->getMoveableLinksRef()[i + 1]->getParentJoint(), getVelocityVector());
        ...
        void getLieBrackets(aligned_vector<LieBracketSE3<T>> &adjs, aligned_vector<Jacobian<T>> &jacobians) {
            for(size_t t_j= 0; t_j < jacobians.size(); t_j++) {
                adjs[t_j]=
                    jacobians[t_j].template block<6, Eigen::Dynamic>(0, 0, 6, t_j + 1) * m_chain->getQd().head(t_j + 1);
            }
        }

        template<Frame OF= F>
        void getJacobiansDq(const aligned_vector<AdjointSE3<T>> &Ads, aligned_vector<Jacobian<T>> &jacs,
            ...
            for(size_t i= 1; i < dof; i++) {
                ...
                for(size_t i0= 0; i0 < dof; i0++) {
                    mt_accumulatorLocal.applyInverseMat(jacsDq[i0][i - 1].template block<6, -1>(0, 0, 6, i),
                                                        jacsDq[i0][i].template block<6, -1>(0, 0, 6, i),
                                                        mt_jacobianDot.template block<6, -1>(0, 0, 6, i));
                }
                // mt_jacobian.template block<6, Dynamic>(0, 0, 6, i)= jacs[i].template block<6, -1>(0, 0, 6, i);
                mt_bodyVelocity= ARDL_visit(m_chain->getMoveableLinksRef()[i]->getParentJoint(), getS());
                mt_bodyVelocity.apply(jacs[i].template block<6, -1>(0, 0, 6, i),
                                      mt_jacobian.template block<6, -1>(0, 0, 6, i),
                                      mt_jacobianDot.template block<6, -1>(0, 0, 6, i));
                jacsDq[i][i].template block<6, Dynamic>(0, 0, 6, i)-= mt_jacobian.template block<6, -1>(0, 0, 6, i);
            }
        }

        template<Frame OF= F>
        void getJacobianDotsDq(const aligned_vector<AdjointSE3<T>> &Ads, aligned_vector<LieBracketSE3<T>> &adjs,
            ...
            for(size_t i0= 0; i0 < m_chain->getNumOfJoints(); i0++) {
                ...
                typename aligned_vector<AdjointSE3<T>>::const_iterator it;
                if constexpr(OF == Frame::BODY) {
                    for(size_t i= 1; i < m_chain->getNumOfJoints(); i++) {
                        Link<T> *link= m_chain->getMoveableLinksRef()[i];
                        Ads[i].applyInverseTo(Ads[i + 1], mt_accumulatorLocal);
                        mt_accumulatorLocal.applyInverseMat(
                            jacDotsDq[i0][i - 1].template block<6, Eigen::Dynamic>(0, 0, 6, i0),
                            jacDotsDq[i0][i].template block<6, Eigen::Dynamic>(0, 0, 6, i0),
                            mt_jacobianDot.template block<6, Eigen::Dynamic>(0, 0, 6, i0));
                        if(i == i0) {
                            mt_bodyVelocity= ARDL_visit(m_chain->getMoveableLinksRef()[i0]->getParentJoint(), getS());

                            mt_accumulatorLocal.applyInverseMat(jacDots[i - 1].template block<6, -1>(0, 0, 6, i0),
                                                                mt_jacobianDot.template block<6, -1>(0, 0, 6, i0),
                                                                mt_jacobianDot.template block<6, -1>(0, 0, 6, i0));

                            mt_bodyVelocity.apply(mt_jacobianDot.template block<6, -1>(0, 0, 6, i0),
                                                  mt_jacobianDot.template block<6, -1>(0, 0, 6, i0),
                        ...
                        mt_bodyVelocity= ARDL_visit(link->getParentJoint(), getVelocityVector());
                        mt_bodyVelocity.apply(jacsDq[i0][i].template block<6, -1>(0, 0, 6, i0),
                                              mt_jacobianDot.template block<6, -1>(0, 0, 6, i0),
                                              mt_jacobianDot.template block<6, -1>(0, 0, 6, i0));
                        jacDotsDq[i0][i].template block<6, -1>(0, 0, 6, i0)-=
                            mt_jacobianDot.template block<6, -1>(0, 0, 6, i0);
        ...
        size_t number_of_collision_checks() {
            ...
            for(size_t i= 0; i < m_chain->getLinksRef().size(); i++) {
                for(t_j= i + 2; t_j < m_chain->getLinksRef().size(); t_j++) {
                    if(m_chain->getLinksRef()[t_j].hasCollision()) { collision_no++; }
                }
            }
            return collision_no;
        }
        ...
        bool collision() {
            int collision_no= 0;
            for(size_t i= 0; i < m_chain->getLinksRef().size(); i++) {
                fcl::CollisionObject<T> *obj1= m_chain->getLinksRef()[i].getCollisionObjectPtr().get();
                for(t_j= i + 2; t_j < m_chain->getLinksRef().size(); t_j++) { // 5 4 3 2 1
                    if(m_chain->getLinksRef()[t_j].hasCollision()) {
                        fcl::CollisionObject<T> *obj2= m_chain->getLinksRef()[t_j].getCollisionObjectPtr().get();
        ...
        const std::vector<std::pair<size_t, size_t>> &getCollisionPairsOptim() {
            ...
            for(size_t i= 0; i < m_chain->getMoveableLinksRef().size(); i++) {
                fcl::CollisionObject<T> *obj1= m_chain->getMoveableLinksRef()[i]->getCollisionObjectOptimPtr().get();
                for(t_j= i + 2; t_j < m_chain->getMoveableLinksRef().size(); t_j++) { // 5 4 3 2 1
                    if(m_chain->getMoveableLinksRef()[t_j]->hasCollision()) {
                        fcl::CollisionObject<T> *obj2=
                            m_chain->getMoveableLinksRef()[t_j]->getCollisionObjectOptimPtr().get();
                        collide(obj1, obj2, mt_request, mt_result);
                        if(mt_result.isCollision()) {
                            t_pair.first= i;
                            t_pair.second= t_j;
